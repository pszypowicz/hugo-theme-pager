...
- Idle main-menu links render as lowercase `./name` paths with an
  ...
  a hover color, which touch screens never show. The current-page
  entry keeps its `>` prompt marker.
- The `# ` prefix on widget titles is now generated by CSS
  (`.widget__title::before`) instead of being hardcoded per template.
  Previously only `toc` and `categories` carried it while `tags` and
  config-supplied `about` titles did not; now every widget title gets
  the same lockup.
